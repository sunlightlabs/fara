...
class Command(BaseCommand):
	def handle(self, *args, **options):
		...
		writer.writerow(["crp_id", "title", "name", "agency", "office", "local"])
		...
		for person in recipients:
			...
			if person.bioguide_id != None:
				...
				if crp[:1] != "N":
					writer.writerow([person.crp_id, person.title, person.name, person.agency, person.office_detail, person.state_local])
