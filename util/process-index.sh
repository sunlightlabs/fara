...
#if [[ ! -d $ROOT_DIR/$1 || -z $1 ]]
#then
#	echo "must give me a real directory ... i may not care if there are files in that directory !"
#	exit 1
#else
#	DEST_DIR=$ROOT_DIR/$1
#fi
...
#
#for FILE in $WERK_DIR/*.pdf
#do 
#	#echo $FILE
#	BASE=$(basename $FILE .pdf)
#	echo $BASE
#	DEST_DIR=$TOP_DEST_DIR/$BASE
#
#	# convert using pdf2htmlEX  ## SIMPLE OPTIONS ## 
#	pdf2htmlEX --embed cfijo --external-hint-tool=ttfautohint --dest-dir $DEST_DIR $FILE
#
#	mv $DEST_DIR/$BASE.html $DEST_DIR/index.html
#	# deal w/ css assets 
#	cat $DEST_DIR/index.html | sed 's/base.min.css/\/css\/base.min.css/' > $DEST_DIR/tmp1	
#	cat $DEST_DIR/tmp1 	 | sed 's/fancy.min.css/\/css\/fancy.min.css/' > $DEST_DIR/tmp2
#
